...
class DirectPromptAgent:
    ...
    def respond(self, prompt):
        ...
        client = _create_client(self.openai_api_key)
            ...
            messages=[
                {
                    "role": "system",
                    "content": (
                        f"Assume this persona exactly: {self.persona}\n"
                        "Forget all previous context. Follow the persona instructions in every answer."
                    ),
                },
...
class RAGKnowledgePromptAgent:
    ...
    def chunk_text(self, text):
        ...
        while start < len(text):
            # -- AI -- Calculate the initial end point for the current chunk.
            ...
            # -- AI -- Fix template infinite loop by stopping at the final chunk and forcing progress.
            if end >= len(text):
                break
...
class EvaluationAgent:
    ...
    # initial_response is optional: pass a pre-generated worker answer to skip the
    # extra respond() call on iteration 0 (otherwise the first run loses content).
    # I had that during my testing, so I added this behavior
    def evaluate(self, initial_prompt, initial_response=None):
        ...
        for i in range(self.max_interactions):
            ...
            if i == 0 and initial_response is not None:
                ...
                print("(Using pre-generated worker response from support function.)")
            else:
...
class ActionPlanningAgent:
    ...
    # Filter preamble lines like 'Development tasks for a product typically include:'
    @staticmethod
    def _clean_steps(response_text):
        steps = []
        for line in response_text.splitlines():
            ...
            looks_like_preamble = (
                re.match(r"^(here are|action steps?|steps?|the (steps?|tasks?|stages?))[:\s]", lower)
                or (
                    ends_with_colon
                    and re.search(
                        r"\b(include|follows?|are|here|step|task|stage|listed|below)\b",
                        lower,
                    )
                )
            )
